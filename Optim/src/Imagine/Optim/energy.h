...
template <typename V, typename T>
inline typename Energy<V,T>::Var Energy<V,T>::add_variable() {	return this->addNode(); }
	...
	*/
	...
	if (B < 0)
	{
		/* Write it as
		   ...
		   0 0     -B 0     B+C 0
		*/
		addTweights(x, 0, B); /* first term */
		addTweights(y, 0, -B); /* second term */
		addEdge(x, y, 0, B+C); /* third term */
	}
	else if (C < 0)
	{
		/* Write it as
		   -C -C  +  C 0  +  0 B+C
		    0  0     C 0     0 0
		*/
		addTweights(x, 0, -C); /* first term */
		addTweights(y, 0, C); /* second term */
		addEdge(x, y, B+C, 0); /* third term */
	}
	else /* B >= 0, C >= 0 */
	{
		addEdge(x, y, B, C);
                              ...
                              Value E110, Value E111)
{
	...
	if (pi >= 0)
	{
		...
		addTweights(x, E101, E001);
		addTweights(y, E110, E100);
		addTweights(z, E011, E010);
	...
	else
	{
		...
		addTweights(x, E110, E010);
		addTweights(y, E011, E001);
		addTweights(z, E101, E100);
...
template <typename V, typename T>
inline int Energy<V,T>::get_var(Var x) { return (int) whatSegment(x); }
