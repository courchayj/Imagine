...
public:
	...
	node_id addNode(int num = 1);

	// Adds a bidirectional edge between 'i' and 'j' with the weights 'cap' and 'rev_cap'.
	// IMPORTANT: see note about the constructor 
	void addEdge(node_id i, node_id j, captype cap, captype rev_cap);
	...
	void addTweights(node_id i, tcaptype cap_source, tcaptype cap_sink);
	...
	termtype whatSegment(node_id i, termtype default_segm = SOURCE);
...
public:
	...
	// After that functions addNode() and addEdge() must be called again. 
	//
	...
	// were added to the graph. NOTE: for each call addEdge(i,j,cap,cap_rev)
	...
	//   addTweights(i),set_trcap(i)    => call mark_node(i)
	//   addEdge(i,j),set_rcap(a)       => call mark_node(i); mark_node(j)
	//
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::node_id Graph<captype,tcaptype,flowtype>::addNode(int num)
{
	assert(num > 0);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::addTweights(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::addEdge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::termtype Graph<captype,tcaptype,flowtype>::whatSegment(node_id i, termtype default_segm)
{
	if (nodes[i].parent)
	{
